...
int	*create_map_token_coord(t_coord *token_joint_cell, \
t_coord *map_joint_cell, int *token_coord, t_dim *map_dim)
{
	int *map_token_coord;
	int r_dif;
	int c_dif;
	...
	map_token_coord = init_arr_coord(map_dim, 2);
	r_dif = map_joint_cell->row - token_joint_cell->row;
	c_dif = map_joint_cell->col - token_joint_cell->col;
	i = 1;
...
int	can_put_on_map(int *map_token_coord, int **map, t_coord *map_joint_cell, t_dim *map_dim)
{
	...
	while (j < map_token_coord[0] && map_token_coord[j] != -2147483648)
	{
		if (((map[map_token_coord[j]][map_token_coord[j + 1]] != -2) && \
			...
			(map[map_token_coord[j]][map_token_coord[j + 1]] == -1 && \
			map_token_coord[j] == map_joint_cell->row && \
			map_token_coord[j + 1] == map_joint_cell->col))
		{
			j = j + 2; 
		}
		else 
...
// ft_printf("\n\n");
	...
	ft_putnbr_fd(map_token_coord[1] - token_coord[1], 1);
	write(1, " ", 1);
	ft_putnbr_fd(map_token_coord[2] - token_coord[2], 1);
	write(1, "\n", 1);
